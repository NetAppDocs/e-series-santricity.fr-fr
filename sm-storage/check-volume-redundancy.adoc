...
NOTE: La fonction Check Volume Redundancy n'est actuellement disponible que pour les opérations dans l'interface Legacy System Manager.

. Dans le panneau de gauche, sélectionnez menu : Storage [Pools and volume groups].
. Cliquez sur l’icône de menu : ellipsis [Check volume redundancy] pour le pool ou le groupe de volumes que vous souhaitez vérifier.
+
