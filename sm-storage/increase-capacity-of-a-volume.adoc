...
. Sélectionnez le volume pour lequel vous souhaitez augmenter la capacité, puis cliquez sur le menu : ellipsis icon[Increase Capacity].
+
La page Confirmer l'augmentation de capacité s'affiche.

. Sélectionnez *Oui* pour continuer.
+
La page « Increase Reported Capacity » s'affiche.

+
Cette page affiche la capacité actuelle signalée du volume et la capacité libre disponible dans le pool ou le groupe de volumes associé au volume.

. Utilisez la case *Augmenter la capacité signalée en ajoutant...* pour ajouter de la capacité à la capacité signalée disponible. Vous pouvez modifier la valeur de capacité à afficher en mébioctets (MiB), gibioctets (GiB) ou tébioctets (TiB).
. Cliquez sur *augmenter*.


.Résultats
* System Manager augmente la capacité du volume en fonction de votre sélection.
* Sélectionnez le menu : Storage[Operations in Progress] pour afficher la progression de l’augmentation de capacité en cours pour le volume sélectionné. Cette opération peut être longue et pourrait affecter les performances du système.
