...
Vous pouvez autoriser System Manager à créer automatiquement des pools lorsqu'il détecte de la capacité non allouée dans une baie de stockage. Sinon, si la création automatique ne permet pas de déterminer la meilleure configuration, vous pouvez créer des pools manuellement à partir du menu:Storage[Pools and volume groups].
...
Vous pouvez créer des groupes de volumes à partir du menu : Storage[Pools and volume groups].
