...
+
Si la capacité disponible n'existe pas sur le pool ou le groupe de volumes contenant l'objet de stockage que vous avez sélectionné et que la baie de stockage dispose d'une capacité non attribuée, vous pouvez ajouter de la capacité. Vous pouvez créer un nouveau pool ou groupe de volumes et réessayer cette opération en utilisant la nouvelle capacité disponible sur ce pool ou groupe de volumes.
...
Sélectionnez le menu : Storage [Operations in Progress] pour afficher la progression de l’opération de rollback.
