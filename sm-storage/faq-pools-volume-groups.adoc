...
+
Pour créer manuellement un pool : sélectionnez *Stockage*, puis *Pools et volume groups*. Dans l’onglet All Capacity, cliquez sur menu :Create[Pool].
...
+
Pour ajouter manuellement des disques à un pool existant : depuis la page Pools and volume groups, sélectionnez le pool, puis cliquez sur *Add capacity*.
...
Vous pouvez modifier la capacité de conservation depuis *Pools and volume groups*. Sélectionnez le pool que vous souhaitez modifier. Cliquez sur l’icône menu:ellipsis icon[Edit Settings], puis sélectionnez l’onglet *Settings*.
...
Dans la boîte de dialogue Ajouter de la capacité, tous les disques ne sont pas disponibles pour ajouter de la capacité à un pool ou un groupe de volumes existant.
...
** Le groupe de volumes doit comporter un nombre pair de disques. Si vous n'avez pas un nombre pair de disques et qu'il vous reste des disques non attribués, accédez à *Pools et groupes de volumes* pour ajouter des disques supplémentaires au groupe de volumes, puis réessayez l'opération.
...
Pour consulter les statistiques du cache SSD, sélectionnez le menu:Storage[Pools et volume groups] dans le panneau de gauche. Sélectionnez le SSD Cache pour lequel vous souhaitez consulter les statistiques, puis cliquez sur le menu:ellipsis icon[View statistics]. Les statistiques nominales sont affichées sur la page View Statistics.
...
[NOTE]
====
Le curseur de capacité d'optimisation supplémentaire est uniquement disponible pour les systèmes de stockage EF80, EF50, EF600 et EF300.
...
L'approvisionnement en ressources est une fonctionnalité disponible dans les baies de stockage EF80, EF50, EF300 et EF600, qui permet de mettre les volumes en service immédiatement sans processus d'initialisation en arrière-plan.
...
L'allocation des ressources est activée par défaut sur les systèmes dont les disques prennent en charge DULBE. Vous pouvez désactiver ce paramètre par défaut depuis *Pools and volume groups*.
