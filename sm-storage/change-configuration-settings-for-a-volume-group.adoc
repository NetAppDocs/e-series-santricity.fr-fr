...
. Sélectionnez le groupe de volume que vous souhaitez modifier, puis cliquez sur l’icône menu:ellipsis icon[Edit Settings].
+
La fenêtre « Modifier les paramètres du groupe de volumes » s'affiche.

. Modifiez les paramètres du groupe de volumes selon vos besoins.
+
.Détails du champ
[%collapsible]
====
