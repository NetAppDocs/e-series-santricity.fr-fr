...
. Cliquez sur l’icône de menu : ellipsis [Change volume cache settings].
+
La fenêtre « Modifier les paramètres du cache de volume » s’affiche. Tous les volumes de la baie de stockage apparaissent dans cette boîte de dialogue.

. Cochez la case à côté de chaque volume pour lequel vous souhaitez modifier les paramètres du cache ou cochez la case dans l'en-tête du tableau pour sélectionner tous les volumes.
+
[cols="25h,~"]
|===
...
Pour un volume fin, la préextraction de lecture dynamique du cache est toujours désactivée et ne peut pas être modifiée.
 ...
 a| 
Mise en cache d’écriture sans batteries
 a| 
...
|===
. Cliquez sur *Enregistrer* pour modifier les paramètres du cache
