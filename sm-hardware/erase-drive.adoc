...
====
. Dans le menu contextuel, sélectionnez *Effacer les lecteurs*.
+
...
. Confirmez l'opération en saisissant `erase`, puis cliquez sur *Effacer les lecteurs*.
+
[CAUTION]
====
...
.Résultats
L'opération d'effacement peut prendre de quelques minutes à plusieurs heures. Vous pouvez consulter l'état dans menu:Storage[Operations in Progress]. Lorsque l'opération d'effacement est terminée, les disques sont disponibles pour une utilisation dans un autre groupe de volumes, un autre pool de disques ou une autre baie de stockage.
