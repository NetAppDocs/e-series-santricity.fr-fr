...
[NOTE]
====
Vous pouvez également générer un fichier CSR à l'aide d'un outil tel que OpenSSL et passer directement à  2: Submit the CSR files.
...
. Accédez à  2: Submit the CSR files.
...
====
. Lorsque l'AC renvoie les certificats signés, accédez à  3: Import signed certificates for controllers.
