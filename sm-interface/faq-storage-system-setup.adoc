...
Vous pouvez consulter les données de performance par application et par charge de travail afin d'évaluer la latence, les IOPS et le MiB/s des applications et de leurs charges de travail associées pour optimiser les performances. La configuration du volume inclut des caractéristiques telles que le type d'E/S, la taille des segments, la propriété du contrôleur, ainsi que les caches de lecture et d'écriture.
