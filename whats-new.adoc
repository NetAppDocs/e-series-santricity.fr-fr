...
[role="lead"]
Le tableau suivant décrit les nouvelles fonctionnalités de SANtricity System Manager 11.8x.
...
La nouvelle taille de clé de certificat par défaut est de 3072 bits (à partir de 2048). Les tailles de clé jusqu'à 4096 bits sont prises en charge. Un bit NVSRAM doit être modifié pour prendre en charge les tailles de clé autres que celles par défaut.
 ...
 a| 
Prise en charge de pools de disques dynamiques de plus grande taille à l'aide de disques avec des tailles de bloc de 512 octets
 a| 
Les clients qui utilisent le type de version de pool de la version 1 peuvent désormais disposer d'un pool de disques de 48 To maximum. L'ancienne version prend en charge un pool de disques de 24 To maximum. Si les pools de disques existants doivent être convertis à la taille la plus grande, tous les volumes doivent être supprimés en premier. Une opération de mise à niveau vers une version antérieure est limitée lorsqu'un pool de disques de la version 1 existe.
